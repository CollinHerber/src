...
public class HandleReturningToCombat implements Task {
    ...
    public boolean validate() {
        return (!Banking.isBankScreenOpen() && !dragonArea.contains(Player.getPosition()) && Inventory.getCount(food) == 15 && Inventory.find(dragonLoot).length == 0) || (bottomOfLadder.isOnScreen() && !dragonArea.contains(Player.getPosition()));
    }
...
private enum TaskState {
    ...
    private TaskState getTaskState () {
        ...
        if (!dragonArea.contains(Player.getPosition()) && !mythsTeleportArea.contains(Player.getPosition()) && !bottomOfLadderArea.contains(Player.getPosition()) && Inventory.getCount(food) == 15 && Inventory.find(dragonLoot).length == 0 && !Banking.isBankScreenOpen()) {
            return TaskState.TELEPORTING_TO_MYTHS_GUILD;
        }
        if (mythsTeleportArea.contains(Player.getPosition()) && !bottomOfLadderArea.contains(Player.getPosition()) && Inventory.getCount(food) == 15 && Inventory.find(dragonLoot).length == 0) {
